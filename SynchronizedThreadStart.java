...
class Worker1 implements Runnable {
    ...
    public void run() {
        try {
            ...
            String curlCommand1 = "curl -X POST -d \"bdarray(filter(events6,i>0));\""+" http:/192.168.0.72:8080/bigdawg/query/ -w " +
            ...
            String curlCommand = "curl -X POST -d \"bdrel(select * from attributes;)\""+" http:/192.168.0.72:8080/bigdawg/query/ -w " +
                    "'{'\"content_type\":\"%{content_type}\",\"filename_effective\":\"%{filename_effective}\"," +
                    "\"ftp_entry_path\":\"%{ftp_entry_path}\",\"http_code\":\"%{http_code}\",\"http_connect\":\"%{http_connect}\"," +
                    "\"http_version\":\"%{http_version}\",\"local_ip\":\"%{local_ip}\",\"local_port\":\"%{local_port}\"," +
                    "\"num_connects\":\"%{num_connects}\",\"num_redirects\":\"%{num_redirects}\"," +
            ...
            //3.Scidb to psql
            String curlCommand5 = "curl -X POST -d \"bdrel(select * from bdcast( bdarray(filter(events1,i>15000)), tab"+Thread.currentThread().getName()+",'(i bigint, eventID int,startFreq real, endFreq real, elevel real, measDist real, eirp real, MeerKATBand text,pol text)', relational))\""+" http:/192.168.0.72:8080/bigdawg/query/ -w " +
                    "'{'\"content_type\":\"%{content_type}\",\"filename_effective\":\"%{filename_effective}\"," +
                    "\"ftp_entry_path\":\"%{ftp_entry_path}\",\"http_code\":\"%{http_code}\",\"http_connect\":\"%{http_connect}\"," +
                    "\"http_version\":\"%{http_version}\",\"local_ip\":\"%{local_ip}\",\"local_port\":\"%{local_port}\"," +
                    "\"num_connects\":\"%{num_connects}\",\"num_redirects\":\"%{num_redirects}\"," +
            ...
            //4.psql to scidb
            String curlCommand2 = "curl -X POST -d \"bdarray(scan(bdcast(bdrel(SELECT * FROM events6scidb limit1), newarray2, '<eventid:double,txid:string,receiverid:string,startfreq:double,endfreq:double,elevel:double,measdist:double,eirp:double,meerkatband:string,pol:string>[i=0:*,10000000,0]',array)));\""+" http:/192.168.0.72:8080/bigdawg/query/ -w " +
                    "'{'\"content_type\":\"%{content_type}\",\"filename_effective\":\"%{filename_effective}\"," +
                    "\"ftp_entry_path\":\"%{ftp_entry_path}\",\"http_code\":\"%{http_code}\",\"http_connect\":\"%{http_connect}\"," +
                    "\"http_version\":\"%{http_version}\",\"local_ip\":\"%{local_ip}\",\"local_port\":\"%{local_port}\"," +
                    "\"num_connects\":\"%{num_connects}\",\"num_redirects\":\"%{num_redirects}\"," +
